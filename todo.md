...
### Inventory の表示フラグ
- [x] Inventory に show_icon フラグを追加（false で何も表示しない）
- [x] Inventory に show_count フラグを追加（false で数量ラベルを隠す）

### 入力バッジ廃止
- [x] 加工機（smelter等）の入力にアイテムを入れてもアイコンが表示されない
- [x] piece.gd から input_hex と入力位置決めを削除する
- [x] Chest の入力バッジは引き続き表示される

### 出力はみ出し表示
- [x] 機械の Output ノードが出力ポート端（中心＋0.5×方向）に配置される
- [x] Output 位置はピース回転に追従する
- [x] 機械の出力は数量ラベルを表示しない（アイコンのみ）
