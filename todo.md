...
- [ ] 単純加工プロセスの実装 (鉄鉱石 -> 鉄板)
	- [x] レシピデータの構造化
		- [x] `Recipe` クラス（または辞書構造）を定義し、入力アイテム、出力アイテム、所要時間を管理できるようにする
		- [x] テスト: レシピデータから期待通りの出力と時間が取得できることを確認
	- [x] 加工機能の実装 (Smelter/Assembler共通ロジック)
		- [x] `Piece` クラスに「現在のレシピ」と「加工進捗」を持たせる
		- [x] `tick` 内で、入力アイテムがあれば消費し、所要時間経過後に出力アイテムを生成するロジックを実装
		- [x] テスト: アイテムAを投入すると、一定時間後にアイテムBがインベントリに追加されることを確認（単体テスト）
	- [x] 精錬機 (Smelter) の設定
		- [x] `WORM` ピースを精錬機とし、入出力ポートと「鉄鉱石 -> 鉄インゴット」のレシピを設定する
		- [x] テスト: `WORM` ピースが鉄鉱石を受け入れ、鉄インゴットを出力ポートから排出することを確認
	- [x] 製作機 (Assembler) の設定
		- [x] `PROPELLER` ピースを製作機とし、入出力ポートと「鉄インゴット -> 鉄板」のレシピを設定する
		- [x] テスト: `PROPELLER` ピースが鉄インゴットを受け入れ、鉄板を排出することを確認
	- [x] 統合テスト (ライン稼働確認)
		- [x] `BAR`(採掘) -> `WORM`(精錬) -> `PROPELLER`(製作) -> `CHEST`(格納) を接続して配置し、最終的にチェストに鉄板が格納されることを確認
...
- [ ] インベントリの分離 (Input/Output)
	- [x] テスト: 加工待ちの材料（Input）は隣接ピースへ搬出されないことを確認する
	- [x] テスト: 加工完了後の成果物（Output）は隣接ピースへ搬出されることを確認する
	- [x] 実装: Pieceクラスに入力・出力インベントリを実装する
	- [x] 実装: 搬入(`add_item`)はInputインベントリへ行う
	- [x] 実装: 搬出(`_push_items_to_neighbors`)はOutputインベントリから行う
	- [x] 実装: 加工(`tick`)はInputから消費しOutputへ生成する
	- [x] 実装: 採掘(`BAR`)はOutputへ生成する

- [ ] Input制約の緩和
	- [x] テスト: Inputポートが定義されていない方向からでも、Outputが合致していればアイテムを受け取れることを確認する
	- [x] テスト: 既存の「Input向き不一致で接続不可」とするテストを修正・削除する
	- [x] 実装: `can_push_to` メソッドのInput側ポートチェックを削除する
