...
### コンベアドラッグ時の発音（1本ごと）
- [ ] ドラッグパスにコンベアが1本追加されるたび PiecePlacer がシグナルを発火する
- [ ] パス追加に失敗したとき（重複ヘックス等）はシグナルを発火しない
- [ ] パス追加シグナルで place_belt.wav が再生される
- [ ] コンベアの piece_placed（リリース時の一括設置）では音を重ねて鳴らさない
- [ ] 統合: main でドラッグパスに追加すると音が鳴る
