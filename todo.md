...
- [ ] Line2D を「パス幾何＋ベルトスプライト」に置換し、forward フレームでベルトを動かす

#### テストリスト
- [x] フレーム index が経過時間で進み 14 で循環する
- [ ] forward フレーム配列が14枚ある
- [ ] _path が3点（in_edge/center/out_edge）を持つ（旧 _line テスト移行）
- [ ] 出力側エッジ点が出力方向にある（旧 _line テスト移行）
- [ ] rotate_cw 後に出力エッジ点が更新される（旧 _line テスト移行）
- [ ] アイテムアイコンがベルト(z=6)より手前(z=7)に描画される（旧 _line.z_index テスト移行）
